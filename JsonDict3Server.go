...
func (t *DIC3) RPCInsertDict3ToSuccessor(dict *map[DKey]map[string]interface{},
reply *int) error {
...
func updateSuccessorOfP() {
        ...
        var reply2 int
        RpcCall := client.Go("DIC3.RPCUpdateSuccessorOfP", successor.Chordid,
&reply2,nil)
        replyCall := <-RpcCall.Done
        if replyCall != nil {
        }
...
func updatePredecessorOfS() {
        ...
        var reply2 int
        RpcCall := client.Go("DIC3.RPCUpdatePredecessorOfS",
predecessor.Chordid, &reply2,nil)
        replyCall := <-RpcCall.Done
        if replyCall != nil {
        }
        client.Close()
...
func purgeUnusedTriplets(){
        ...
        for k, v := range dict3{
                ...
                if(strings.EqualFold("RW", access)){
                        ...
                        access_time, err := time.Parse("2006-01-02 15:04",
accessed)
                        checkError(err)
...
func stabilize(){
        stabilizeTicker := time.NewTicker(time.Duration(stabilizeInterval) *
time.Second)
        ...
        go func() {
                for {
                        ...
                        case <-stabilizeTicker.C:
                                fmt.Println("stabilizing chord ring ",
time.Now())
                                client, err := jsonrpc.Dial(protocol,
successor.Address)
                        checkError(err)
                        var x NodeInfo
                        RpcCall := client.Go("DIC3.Predecessor", successor,
&x,nil)
                        replyCall := <-RpcCall.Done
                        if replyCall != nil {}

                                if(x.Chordid> chordid && x.Chordid <
successor.Chordid){
                                ...
                                //its predecessor
                                RpcCall = client.Go("DIC3.Notify", successor,
nil,nil)
                        replyCall = <-RpcCall.Done
                        if replyCall != nil {}
...
predecessor.Address)
                        ...
                        //var x NodeInfo
                        //RpcCall := client.Go("DIC3.Predecessor", ipAdd,
                        //&x,nil)
                        //replyCall := <-RpcCall.Done
                        //if replyCall != nil {}
        ...
        go func() {
                for {
                        ...
                        case <-fixfingersTicker.C:
                                //fmt.Println("fix fingers of node ",
                                //time.Now())
                                next = next + 1
                                if next > 5 {
                                        next = 1
